...
class Solver:
    ...
    # Find what color should at this location and if the board is still solvable
    def __check_adjacents(self, x, y):

        # Left
        if ( y - 2 >= 0 ):
            if ( self.board[x][y-1] == self.board[x][y-2] and self.board[x][y-1] != NONE):
                if(not self.__assert_should_be(x,y,get_opposite(self.board[x][y-1]))):
        ...
        # Up 
        if ( x - 2 >= 0 ):
            if ( self.board[x-1][y] == self.board[x-2][y] and self.board[x-1][y] != NONE):
                if(not self.__assert_should_be(x,y,get_opposite(self.board[x-1][y]))):
                    return False # Not solvable
        ...
        if ( x + 1 < self.size and x - 1 >= 0 ):
            if ( self.board[x+1][y] == self.board[x-1][y] and self.board[x+1][y] != NONE):
                if(not self.__assert_should_be(x,y,get_opposite(self.board[x+1][y]))):
                    return False # Not solvable

        # Middle Horizontal
        if ( y + 1 < self.size and y - 1 >= 0 ):
            if ( self.board[x][y+1] == self.board[x][y-1] and self.board[x][y-1] != NONE):
                if(not self.__assert_should_be(x,y,get_opposite(self.board[x][y-1]))):
                    return False # Not solvable
        ...
        for n in nums:
            ...
            if(to_check[to_check == NONE].size == 0):
                ...
                if(current_diff.size < 2):
                    return False # No way for it to be less than two at this point
                elif(current_diff.size == 2 and current_diff[0] == NONE and current_diff[1] == NONE):
    ...
    def __check_all_row_col(self):
        getCol = lambda col : self.board[:,col]
        getRow = lambda row : self.board[row,:]

        for n in range(self.size):
            for func in [getCol, getRow]:
    ...
    def __is_valid(self, x, y, color):
        ...
        # Left
        if ( y - 2 >= 0 ):
            if ( self.board[x][y-1] == self.board[x][y-2] and self.board[x][y-1] == color):
                return False
        ...
        # Up 
        if ( x - 2 >= 0 ):
            if ( self.board[x-1][y] == self.board[x-2][y] and self.board[x-1][y] == color):
                return False
        ...
        # Middle Vertical
        if ( x + 1 < self.size and x - 1 >= 0 ):
            if ( self.board[x+1][y] == self.board[x-1][y] and self.board[x+1][y] == color):
                return False

        # Middle Horizontal
        if ( y + 1 < self.size and y - 1 >= 0 ):
            if ( self.board[x][y+1] == self.board[x][y-1] and self.board[x][y-1] == color):
                return False
        ...
        if( row[row == color].size >= self.size / 2 or col[col == color].size >= self.size / 2 ):
            return False

        for row_n in range(self.size):
            test_row = self.board[row_n,:]
            diff = test_row != row
            if(test_row[test_row == NONE].size == 0 and test_row[diff].size == 1 and test_row[diff][0] == color):
                return False

        for col_n in range(self.size):
            test_col = self.board[:,col_n]
            diff = test_col != col
            if(test_col[test_col == NONE].size == 0 and test_col[diff].size == 1 and test_col[diff][0] == color):
                return False
    ...
    def __assert_should_be(self, x, y, color):
        ...
        if(isColor == NONE):
            ...
            return False # We are somewhere we shouldn't be, unable to complete the board
